...
class DbCarteirinha
{
    ...
    public function selectValidaCarteirinha($cpf, $ra)
    {
        $query = "SELECT PPE.n_identificador IDENTIFICADOR
        FROM pessoas PPE LEFT JOIN
             pessoas_varios_numeros PVN ON PPE.id = PVN.pessoa_id 
WHERE (PPE.CPF = '$cpf' OR PVN.numero = '$ra') AND estado IN (0,1,2) AND PPE.n_identificador IS NOT NULL
        ";
        return $this->exculte($query);
    }
}
